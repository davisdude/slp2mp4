...
def test_existing_file_artifact_happy_path(make_file):
    make_file("text.txt", create=True, cls=artifact.ExistingFileArtifact)


def test_existing_file_artifact_sad_path(make_file):
    with pytest.raises(RuntimeError, match=r"^'.*text\.txt' does not exist\.$"):
        make_file("text.txt", create=False, cls=artifact.ExistingFileArtifact)

    with pytest.raises(RuntimeError, match=r"^'.*game\.slp' does not exist\.$"):
        make_file("game.slp", create=False, cls=artifact.SlippiArtifact)


def test_extension_checking(make_file):
    make_file("game.slp", create=True, cls=artifact.SlippiArtifact)

    with pytest.raises(
        RuntimeError,
        match=r"^'.*game\.xyz' has invalid file extension for a slippi file\.$",
    ):
        make_file("game.xyz", create=True, cls=artifact.SlippiArtifact)

    make_file("vid.mp4", create=True, cls=artifact.Mp4Artifact)

    with pytest.raises(
        RuntimeError,
        match=r"^'.*vid\.xyz' has invalid file extension for an mp4 file\.$",
    ):
        make_file("vid.xyz", create=True, cls=artifact.Mp4Artifact)

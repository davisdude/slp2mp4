...
from slp2mp4.output import OutputComponent
...
def render(
    ...
    if component.context:
        ...
        sb = conf["scoreboard"]["type"](component.context, conf, height)
    ...
    with (
        ...
        if conf["runtime"]["debug"]:
            for i in inputs:
                shutil.copyfile(i, final_path.parent / f"{component.slp.name}_{i.name}")
        ...
        if conf["runtime"]["debug"]:
            shutil.copyfile(
                output_path, final_path.parent / f"{component.slp.name}_{output_path.name}"
            )

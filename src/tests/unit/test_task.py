...
def test_extension_checking(make_file):
    ...
    with pytest.raises(RuntimeError, match=r"^'.*game\.xyz' has invalid file extension for a slippi file\.$"):
    ...
    with pytest.raises(RuntimeError, match=r"^'.*vid\.xyz' has invalid file extension for an mp4 file\.$"):
